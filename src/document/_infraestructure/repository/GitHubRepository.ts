...
export class GitHubRepository implements ContentRepository {
  ...
  > {
    ...
    for (const file of contentFiles) {
      ...
      try {
        ...
        documents.push(
          (await import("../../_domain/Document")).Document.create(
            id,
            data,
            body
          )
  ...
  async get(
    ...
    return (
      documents.find((document) => document.getId().getValue() === id.getValue()) ?? null
    );
  }

  async getBySlug(slug: string): Promise<import("../../_domain/Document").Document | null> {
    const documents = await this.list();
    return (
