...
export class ParsePath {
  ...
  resolveContentDir(contentDir: string = DEFAULT_CONTENT_DIR): string {
  ...
  idToFilePath(
    ...
    const contentRoot = this.resolveContentDir(contentDir);
  ...
  filePathToId(
    ...
    if (withoutExt.endsWith("/index")) {
      return new DocumentId(withoutExt.slice(0, -"/index".length) || "index");
    }

    return new DocumentId(withoutExt);
  }
}
